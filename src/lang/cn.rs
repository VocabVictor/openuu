...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("Waiting for the peer to accept", "等待对方接受"),
        ("Connected via direct connection", "已连接 · 直连"),
        ("Connected via relay", "已连接 · 中继"),
        ("Disconnected, reconnecting in {} s", "已断开，{} 秒后重连"),
        ("Reconnect now", "立即重连"),
        ("Direct", "直连"),
        ("Relayed", "中继"),
        ("Packet loss", "丢包"),
        ("packet-loss-unavailable-tip", "当前连接方式统计不到丢包：TCP 与中继会自行重传，KCP 暂未上报重传次数。"),
        ("ctrl-alt-del-unavailable-tip", "被控端需要以服务方式运行才能接收 Ctrl + Alt + Del。请在被控端安装为服务，或先请求提权。"),
        ("Online", "在线"),
        ("Offline", "离线"),
        ("Status unknown", "状态未知"),
        ("Quick launch", "快速启动"),
        ("quick-launch-empty-tip", "把被控端常用的程序放在这里，以后连接时一次点击就能打开。"),
        ("Refresh shortcuts", "刷新快捷方式"),
        ("Start assistance", "开始协助"),
        ("Connect to a partner", "远控伙伴设备"),
        ("Partner device ID", "伙伴的设备 ID"),
        ("Enter device ID", "请输入设备 ID"),
        ("Recent connections", "最近连接"),
        ("Tap a device to connect again", "点击设备可再次连接"),
        ("No recent connections", "暂无最近连接"),
        ("This device", "本设备"),
        ("Not ready", "未就绪"),
        ("Allow remote assistance", "允许他人远程协助"),
        ("This device ID", "本设备 ID"),
        ("Copy and share", "复制并分享"),
        ("Share as QR code", "二维码分享"),
        ("Cannot build the QR code", "无法生成二维码"),
        ("Scanning with OpenUU on a phone connects to this device.", "手机 OpenUU 扫码后会连接本设备。"),
        ("Authentication configured", "已配置验证方式"),
        ("Show / hide password", "显示／隐藏验证码"),
        ("Refresh password", "刷新验证码"),
        ("Authentication settings", "验证设置"),
        ("Uses your security settings", "连接时按安全设置验证"),
        ...
        ("Move down", "下移"),
        ("Move up", "上移"),
        ("No online LAN helper has discovered this device", "需要已发现目标的同账号在线局域网节点"),
        ("Not available for this device", "当前设备暂不支持"),
        ("Reorder", "调整顺序"),
        ("Sign in on the target device first", "请先在目标电脑登录一次"),
        ("Wake request failed. Check your login and LAN helper.", "唤醒请求失败，请检查登录和在线代发节点。"),
        ("Wake requested. Wait for the device to come online; WOL and standby power are required.", "已请求发送唤醒包，请等待设备上线。目标需启用 WOL 并保持供电。"),
        ("Wake service unavailable", "唤醒服务暂不可用"),
        ("Wake through an online LAN device", "由在线局域网设备代发唤醒包"),
        ("Wake up", "远程开机"),
        ("Files", "文件传输"),
        ("View only", "观看模式"),

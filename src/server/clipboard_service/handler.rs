...
impl Handler {
    ...
    pub(super) fn check_clipboard_file(&mut self) {
    ...
    pub(super) fn get_clipboard_msg(&mut self) -> Option<Message> {
    ...
    #[cfg(windows)]
    pub(super) fn read_clipboard_from_cm_ipc(&mut self) -> ResultType<Vec<ClipboardNonFile>> {
        if self.rt.is_none() {

...
### 1.4 判定（已被后续事实取代，保留原判断以便对照）

**现状（2026-09-13 晚）：Android 恢复可构建，CI 能产出内容完整的 APK。**
`.github/workflows/android-build.yml` 第三轮全绿（run 34755672904）：
`app-arm64-v8a-release.apk` 28.6 MB，断言步骤用 `unzip -l` 确认包内
`lib/arm64-v8a/librustdesk.so`（36,087,544 字节）与 `libc++_shared.so`（1,822,720 字节）
都在，与打包前 `ls -l` 的尺寸一致。所以下面这段"一启动就崩"的判断对当前 master 不再成立。

**这句话的边界要说清楚**：CI 证明的是**包的内容完整**，不是**这个包能用**。
真机安装与运行至今没有做过，界面、连接、权限流程都未经验证。
在有人真的装一次之前，"移动端可构建"是事实，"移动端可用"不是。

走到今天用了三轮，每轮都是不同的真问题：第一轮死在编 Rust 核心，暴露出两处
**从未被编译过的代码缺陷**（`07a0dc2c3`、`aea6320f2`）；第二轮 Rust 过了、死在打包，
是 Gradle 堆 1024M 不够，R8 minify、签名校验、lint 三个任务同时 OOM（`c0f2e791a` 提到 4096M）；
第三轮全绿。
...
| ~~P1~~ **已完成** | 恢复移动端可构建 | 已完成：`android-build.yml` 第三轮全绿并断言了包内两个 `.so`。触发方式是 path-scoped push 加手动，不定时。**边界见 §1.4：可构建 ≠ 可用，真机未验证** |

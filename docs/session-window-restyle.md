...
设计自查（0f 要求，对照 design-review 与 `docs/cm-restyle-plan.md` 的信任界面规则）后的三处调整：

* 状态条由方案里的 **28 高改为 32**，内嵌的"立即重连 / 断开"按钮命中区由 20 提到 24
  （`506881be6`）。20 低于指针目标应有的 24，而 28 的条高放不下带内边距的 24；0f 裁定
  可达性优先于紧凑。两个按钮同尺寸、同样有边框，安全的一侧不会比结束会话的一侧更难点。
* tab、工具栏、窗口三处关闭按钮的 hover 保持不同（6% 黑 / 危险浅底 / 12% 危险色），
  理由写进了 `theme.dart`：tab 关闭只关一个标签，另两个会结束会话，危险度不同。
* `UiDialog` 的 Esc 收紧为只走 `onClose`，不再回退到第一个 secondary 动作
  （`3249bd8e0`，见当日 changelog 的 Defects 一节）。

...
{
  HINSTANCE hInstance = LoadLibraryA("libopenuu.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libopenuu.dll." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  // Keep the OpenUU UI separate from a running upstream RustDesk window.
  app_name = L"OpenUU";
  ...
  // Compute window bounds for default main window position: (10, 10) x(1100, 800)
  ...
  Win32Window::Size size(1100u, 760u);
